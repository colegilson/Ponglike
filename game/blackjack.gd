class_name Blackjack extends Node2D
...
func _ready() -> void:
	if SaveData.left_river and not SaveData.cancel_river: winner = "left"
	if SaveData.right_river and not SaveData.cancel_river: winner = "right"
	if winner == "left":
...
func _process(_delta: float) -> void:
	if right_score == 0 and right_hand.get_child(0).text != "Bust!":
		print("here")
		right_hand.get_child(0).text = "Bust!"
		return 
	if left_score == 0 and left_hand.get_child(0).text != "Bust!":
		left_hand.get_child(0).text = "Bust!"
		return

	if (right_hand.get_child_count() > 4 or right_score == 21 or Input.is_action_just_released("right_move_down")) and (right_hand.get_child(0).text != "Bust!" and right_hand.get_child(0).text != "held at: %s" % right_score):
	...
	if (left_hand.get_child_count() > 4 or left_score == 21 or Input.is_action_just_released("left_move_down")) and left_hand.get_child(0).text != "Bust!" and left_hand.get_child(0).text != "held at: %s" % left_score: 
...
func wait_for_space() -> void:
	if right_score < left_score:
		win_text.set_text("Player 1 is the winner!")
	elif right_score > left_score:
		win_text.set_text("Player 2 is the winner!")
	else:
		win_text.set_text("Its a draw! You both win")
